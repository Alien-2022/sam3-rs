...
def save_results_from_single_inference(image, result, base_name, colors=COLORS):
    """
    ...
    # Save semantic head result
    if result.semantic_pred is not None:
        semantic_pred = result.semantic_pred.cpu().numpy()
        base_path = f"{base_name}_semantic_head"
        print(f"  Saving semantic_head results...")
        save_prediction_mask(semantic_pred, f"{base_path}_pred.png")
        save_colored_mask(semantic_pred, colors, f"{base_path}_color.png")
        save_overlay(image, semantic_pred, colors, f"{base_path}_overlay.png")

    # Save instance head result
    if result.instance_pred is not None:
        instance_pred = result.instance_pred.cpu().numpy()
        base_path = f"{base_name}_instance_head"
        print(f"  Saving instance_head results...")
        save_prediction_mask(instance_pred, f"{base_path}_pred.png")
        save_colored_mask(instance_pred, colors, f"{base_path}_color.png")
        save_overlay(image, instance_pred, colors, f"{base_path}_overlay.png")

    # Save dual head result (final fused result)
...
def run_single_inference(segmentor, image_path, output_dir, img_name):
    """
...
def main():
    ...
    run_single_inference(segmentor, test_image_path, output_dir, img_name)

...
class SAM3RSSegmentor:
    ...
    def __init__(self, config: InferenceConfig):
        ...
        if config.use_semantic_enhancement:
            self._apply_semantic_enhancement()

        # Pre-compute text features for all prompts to avoid repeated computation
        self.text_features_cache = None
        self._precompute_text_features()

        # Initialize presence score statistics if enabled
        self.presence_score_stats = []
        if config.analyze_presence_score:
            print("✓ Presence score analysis enabled")
            print(f"  use_presence_score: {config.use_presence_score}")
            print(f"  presence_score_mode: {config.presence_score_mode}")
    ...
    def print_presence_score_stats(self):
        """Print statistics of collected presence scores."""
        if not self.config.analyze_presence_score:
            print("[DEBUG] analyze_presence_score is False")
            return

        if not self.presence_score_stats:
            print("[DEBUG] presence_score_stats is empty")
            return

        import pandas as pd

        stats = self.presence_score_stats
        all_scores = [s["presence_score"] for s in stats]
        ...
        # Per-prompt statistics (top 10 by average score)
        prompt_stats = {}
        for s in stats:
            prompt = s["prompt"]
    ...
    def _apply_semantic_enhancement(self):
        """
        ...
        """
        if not self.prompts or self.num_classes == 0:
            return
        ...
        class_prompts = {}
        for prompt_name, class_id in zip(self.prompts["names"], self.prompts["indices"]):
        ...
        self.prompts["names"] = enhanced_prompts_names
        self.prompts["indices"] = enhanced_prompts_indices
        self.num_prompts = len(enhanced_prompts_names)

        # Re-convert class indices to tensor
        self.query_indices = torch.tensor(
            self.prompts["indices"], dtype=torch.int64, device=self.device
        )

        # Store synonym counts for reference
        self.class_synonym_counts = class_synonym_counts

        print(f"✓ Semantic enhancement applied: {self.num_classes} classes, {self.num_prompts} enhanced prompts")
    ...
    def _inference_single_view(
        ...
        with torch.no_grad(), torch.autocast(
            ...
            for prompt_idx, prompt_word in enumerate(self.prompts["names"]):
                ...
                # Collect presence score statistics if enabled
                if self.config.analyze_presence_score:
                    ps = output.get("presence_score", 1.0)
                    self.presence_score_stats.append({
                        "image": image_name,
                        "prompt": prompt_word,
                        "presence_score": ps
                    })
                ...
                # NOTE: When presence_score_mode is "after_fusion", apply to fused result (both heads)
                # When presence_score_mode is "before_fusion", semantic head already applied it above
        ...
        with torch.inference_mode(), torch.autocast(
            ...
            for prompt_idx, prompt_word in enumerate(self.prompts["names"]):
                ...
                if self.config.analyze_presence_score:
                    ...
                    if prompt_idx == 0:
                        print(f"[DEBUG] First prompt processing: batch_size={len(img_presence)}, image_names={len(image_names) if image_names else 'None'}")
                    for img_idx, ps in enumerate(img_presence):
                        self.presence_score_stats.append({
                            "image": image_names[img_idx] if image_names else f"img_{img_idx}",
                            "prompt": prompt_word,
                            "presence_score": ps.item()
                        })
                    if prompt_idx == 0:
                        print(f"[DEBUG] Collected {len(self.presence_score_stats)} stats so far")
                ...
                if self.config.use_semantic_head:
                    ...
                    # Apply presence score to semantic head if enabled
                    # Note: instance head already includes presence_score in SAM3's internal computation (line 700)
                    if self.config.use_presence_score and self.config.presence_score_mode == "before_fusion":

...
def main() -> None:
    ...
    for batch in loader:
        ...
        masks = batch["mask"]
        for img_path, gt_mask in zip(paths, masks):
            result = segmentor.predict_single(img_path, detailed=False)
            ...
            #     print(
            #         f"[WARN] pred out of range in {img_path}: min={pred_np.min()} max={pred_np.max()} num_classes={dataset.num_classes}"
            #     )
            # if gt_np.max() >= 100:  # catch potential noisy labels (e.g., 255)
            #     print(
            #         f"[WARN] gt has high values in {img_path}: min={gt_np.min()} max={gt_np.max()}"
            #     )

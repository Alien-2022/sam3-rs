"""iSAID dataset loader for evaluation."""
...
class iSAIDDataset(Dataset):
    def __init__(
        ...
        """
        iSAID Dataset for semantic segmentation evaluation.
        ...
        Args:
            ...
            reduce_zero_label: Whether to subtract 1 from labels to map background(0) to -1
                             iSAID uses labels 0-15 (0=background, 1-15=classes),
                             set to True to map to -1/0-14 and ignore background in metrics
        """
    ...
    def __getitem__(self, idx: int):
        ...
        # iSAID masks are stored as PNG format with dtype uint8
        # Labels are 0-15 (0=background, 1=large vehicle, ..., 15=storage tank)
        mask = np.array(Image.open(mask_path), dtype=np.int64)

        if self.reduce_zero_label:
            # Map labels 0-15 to -1/0-14:
            #   - background(0) -> -1 (will be marked as ignore_index)
            #   - large vehicle(1) -> 0
            #   - ...
            #   - storage tank(15) -> 14
            mask = mask - 1

"""OpenEarthMap dataset loader for evaluation."""
...
def _index_by_stem(paths: Iterable[Path]):
    ...
    :param paths:
        [
            WindowsPath('D:/data/OpenEarthMap/Exp/img/aachen_11.tif'),
            WindowsPath('D:/data/OpenEarthMap/Exp/img/aachen_39.tif')
        ]
    :return:
        {
            'aachen_11': WindowsPath('D:/data/OpenEarthMap/Exp/img/aachen_11.tif'),
            'aachen_39': WindowsPath('D:/data/OpenEarthMap/Exp/img/aachen_39.tif')
        }
    """
...
class OpenEarthMapDataset(Dataset):
    def __init__(
        ...
        """
        OpenEarthMap Dataset for semantic segmentation evaluation.
        ...
        Args:
            ...
            reduce_zero_label: Whether to subtract 1 from labels to map 1-9 to 0-8
                             OpenEarthMap uses labels 1-9 for 9 classes, set to True
                             to map to 0-8 for compatibility
        """
        ...
        # In the constructor of OpenEarthMapDataset, users can pass in a text file path through cls_file
    ...
    def __getitem__(self, idx: int):
        ...
        # OpenEarthMap masks are stored as TIFF format with dtype uint8
        # Labels are 1-9 for 9 classes (background is 1, not 0)
        mask = np.array(Image.open(mask_path), dtype=np.int64)

        if self.reduce_zero_label:
            # Map labels 1-9 to 0-8 for compatibility with models that expect 0-indexed labels
            mask = mask - 1
